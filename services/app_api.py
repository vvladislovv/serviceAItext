    ...
    try:
        ...
        for user in users:
            ...
            if user_id:
                ...
                await add_to_table(
                    "StaticAIUsers", {"chatId": user_id, "dataGpt": ai_state}
                )
...
async def update_subscription(
    sub_data: SubscriptionUpdate, api_key: str = Depends(verify_api_key)
):
    ...
    try:
        ...
        update_data = {"chatId": sub_data.user_id, "tarif": sub_data.tariff}
...
async def get_usage_stats(
    time_range: TimeRange = Depends(), api_key: str = Depends(verify_api_key)
):
    ...
    try:
        ...
        for h in filtered:
            h_date = (
                ...
                else datetime.fromisoformat(h["timestamp"])
            )
            date_str = h_date.strftime("%Y-%m-%d")
            requests_by_day[date_str] = requests_by_day.get(date_str, 0) + 1

        response_times = [
            h.get("response_time", 0) for h in filtered if h.get("response_time")
        ]
        avg_response_time = (
...
async def get_chat_history(
    user_id: int, limit: int = 10, api_key: str = Depends(verify_api_key)
):
    ...
    try:
        ...
        return ChatHistory(
            user_id=user_id, messages=messages, total_messages=len(history)
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def get_user_detail(user_id: int, api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        favorite_model = (
            max(model_counts.items(), key=lambda x: x[1])[0] if model_counts else "none"
        )
